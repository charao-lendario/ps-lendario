...
DECLARE
  adavio_id uuid;
  event_date date := '2025-10-31'::date;
...
BEGIN
  ...
  IF adavio_id IS NOT NULL THEN
    ...
    WHILE event_date <= end_date LOOP
      day_of_week := EXTRACT(DOW FROM event_date); -- 0=Sunday, 1=Monday, etc.
      
      -- Monday (1), Wednesday (3), Friday (5) at 18:30
      IF day_of_week IN (1, 3, 5) THEN
        INSERT INTO public.events (date, time, guest_id, status, type)
        VALUES (event_date, '18:30:00', adavio_id, 'scheduled', 'technical');
      END IF;
      
      -- Tuesday (2), Thursday (4) at 10:00
      IF day_of_week IN (2, 4) THEN
        INSERT INTO public.events (date, time, guest_id, status, type)
        VALUES (event_date, '10:00:00', adavio_id, 'scheduled', 'technical');
      END IF;
